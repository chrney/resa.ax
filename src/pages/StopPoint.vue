<template>
  <div class="q-pa-md row q-col-gutter-md" v-if="stop">
    ...
    <div class="col-12">
      ...
      <template v-if="schedules.length > 0">
        <p v-for="(schedule, s_key) in schedules" v-bind:key="s_key">
        <ul>
          <li>Linje
            <q-icon :name="$mode_to_icon(routes[schedule.pattern.routeId].mode)"/>
            <q-chip>{{ routes[schedule.pattern.routeId].shortName }}</q-chip>
            <strong>{{
                routes[schedule.pattern.routeId].longName
              }}</strong> | <a class="cursor-pointer" @click="getPatternFn(schedule.pattern.id)">Visa färdplan</a>
          </li>
          <li v-for="(t, t_idx) in schedule.times" v-bind:key="t_idx">
            <q-icon name="schedule"/>
            {{
              $moment((t.serviceDay * 1000) + (t.scheduledDeparture * 1000)).format('HH:mm')
...
import {Dialog, QChip} from "quasar";
...
export default defineComponent({
  ...
  setup: function () {
    ...
    const get_schedules = (dateStr) => {
      api.get('index/stops/' + id + '/stoptimes/' + dateStr).then(response => {
    ...
    const changeDateFn = (value, reason, details) => {
      get_schedules(value)
    }
